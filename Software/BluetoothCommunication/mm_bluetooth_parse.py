...
def command_line_parse(command):
    ...
    match translation:
        case c.SET_MODE:
            ...
            response = response_parse(bluetooth_receive(2))           
        case c.HALT_RUN:
            response = "stop OK"
        case c.RESUME_RUN:
            response = "resume OK"
...
def bluetooth_connect():
    try:
        ...
        bl_sock = serial.Serial(param.PORT, baudrate=param.BAUD, timeout=param.TIMEOUT)
        param.SOCKET.flushOutput()
        param.SOCKET.flushInput()
...
def receive_and_update(ACK=None):
    if param.MODE:
        ...
        if specifier == "Debug":
            ...
            param.MOUSE_DIRECTION = param.DIRECTIONS[struct.unpack('B', data[265])[0]]
            param.MOUSE_POSITION = [struct.unpack('B', (data[266] >> 4) & 0x0F)[0], struct.unpack('B', data[266] & 0x0F)[0]]
